...
class Pipeline(BaseModelTool):
    ...
    def instagram(self) -> InstagramTool:
        if self._instagram is None:
            ...
            # VALID PRODUCTION VALUES (Ensures stability in GitHub Actions)
            gcp_project_id = "automatizacion-475715"
            gcs_bucket_name = "enigma-reels-storage"
            
            Messenger.info(f"🔧 Instagram Production Config: Project={gcp_project_id}, Bucket={gcs_bucket_name}")
    ...
    def step2_generate_images(self):
        ...
        4. Updates state.
        """
        # 1. Retrieves idea that needs images (could be SCRIPT_GENERATED or even later if images are missing)
        idea_obj = self.store.get_first_by_state(State.SCRIPT_GENERATED)
        if not idea_obj:
            # Check if we are in a later state but images are missing
        ...
        if not idea_obj:
            Messenger.error("No project found that needs image verification/generation.")
            return

        Messenger.info(f"\n--- Verifying/Generating images for Idea {idea_obj.id} ---")
        ...
        for i, scene in enumerate(script_data.scenes):
            ...
            if alt_path.exists():
                 Messenger.info(f"Skipping Scene {i+1}: {alt_path.name} already exists (alternative extension).")
                 continue
    ...
    def step3_generate_audios(self):
        ...
        for start_idx in range(0, total_scenes, batch_size):
            try:
                ...
                # BLINDAJE: Solo enviar texto a Vertex, prompt a Gemini
                from tools.audio_generation.vertex_ai_tts import VertexAIAudioGenerator
    ...
    def step4_generate_videos(self):
        ...
        for i in range(len(script_data.scenes)):
            # Check for both .mp4 and .png as source
            mp4_path = self.get_idea_asset_path(
                idea_obj.id, self.IMAGES_DIR, f"scene_{i+1}.mp4"
            )
            png_path = self.get_idea_asset_path(
                idea_obj.id, self.IMAGES_DIR, f"scene_{i+1}.png"
            )
            
            source_path = mp4_path if mp4_path.exists() else png_path
            
            if not source_path.exists():
                Messenger.error(f"Missing source for Scene {i+1} (checked .mp4 and .png)")
                continue
    ...
    def step5_generate_subtitles(self):
        ...
        # 4.5. Colorize keywords (Bright Orange: #FF8C00)
        keywords = ["cerebro", "neuronas", "dopamina", "cuerpo", "biología", "secreto", "biohacking", "sistema", "nervioso", "sangre", "celular"]
        if subs_srt.exists():
            import re
            srt_text = subs_srt.read_text(encoding="utf-8")
            for kw in keywords:
                srt_text = re.sub(f"(?i)({kw})", r'<font color="#FF8C00">\1</font>', srt_text)
    ...
    def generate_instagram_description(self, title: str) -> str:
        """
    ...
    def step8_upload_to_instagram(self):
        """
        Upload to Instagram: Uploads all COMPLETED videos to the configured Instagram Account as Reels.
        1. Retrieves all COMPLETED ideas.
        2. For each idea:
            a. Generates an AI-optimized description.
            b. Finds the final named video.
            c. Uploads via InstagramTool.
        ...
        while True:
            ...
            description = self.generate_instagram_description(video_title)

            # 4. Uploads via InstagramTool.
            try:
